...
struct fdtdSource
{
  ...
  double truncated_sawtooth() const {
  ...
  double truncated_squarewave() const {
  ...
  void setPPW(int ppw) {
    this->ppw = ppw;
    this->radiansPerTimestep = two_pi * courant_factor / this->ppw;
  }

  void updateTheta() {
    theta += radiansPerTimestep;
    if (thetaWraparound) {
